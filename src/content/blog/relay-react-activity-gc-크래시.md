...
> 메인 피드의 성능을 높이기 위해 Activity를 다시 도입하기 위해 원인과 결과를 분석하고 팀에 공유한 내용을 기록해요 
...
당근부동산의 메인 페이지는 **리스트(List)와 지도(Map) 두 가지 뷰**를 토글 형식으로 전환하는 구조예요. 문제는 지도 뷰가 초기 렌더에 불필요하게 포함되어 있었다는 점이에요. 사용자가 리스트만 보고 있어도 지도 컴포넌트가 함께 마운트되어 초기 로딩 성능을 잡아먹고 있었어요.
...
센트리에는 유저가 어떤 flow를 탔었는지 영상으로 record 해주는 기능이 존재해요. 허나 웹뷰의 경우 성능과 메모리상의 이유로 해당 기능을 사용하고 있지 않았고 유저의 로그를 통해서만 확인을 할 수 있는 상황이었어요. 


크래시가 난 유저들의 대부분의 공통점은 한가지였어요 

> 여러 페이지를 방문한 Heavy 유저

여러 페이지를 방문했다면 의심할 수 있는것은 메모리 이슈와 relay의 GC가 올바르게 되지 않는가? 였어요 
...
웹뷰 메인 페이지에서 **지도(Map) ↔ 리스트(List) Activity 전환** 시 발생했던 크래시의 예시 애러 트레이스에요.
...
relay의 query로 가져온 데이터가 비어있었고 당연히 데이터가 있을것이라고 가정되어 있어 가드가 없는 데이터는 크래시 되게 되었습니다. 
...
Relay는 저도 팀에 합류하면서 처음 써보게 되는 프레임워크라 다소 학습에 시간이 필요하게 되었어요. 
...
Relay의 Store는 기본적으로 두가지 핵심 키워드를 가져가야해요 

1. releaseBuffer는 “최근에 해제된 쿼리 루트(operation id)”를 잠깐 보관하는 캐시
2. gcScheduler는 “실제 GC 작업을 언제/어떻게 나눠 실행할지”를 담당

**> releaseBuffer가 왜 필요할까요 ?** 

1. dispose()로 refCount가 0이 되자마자 바로 지우면, 화면 왕복(뒤로가기/탭 전환) 때 같은 데이터를 다시 fetch할 가능성이 커요!
2. 그래서 Relay는 0이 된 루트를 releaseBuffer에 넣어 잠깐 살려둘 수 있어요
3. 버퍼 용량을 넘을 때만 오래된 것부터 GC 대상으로 넘깁니다.
4. 기본 버퍼 크기는 10입니다.

**> gcScheduler(질문하신 gc 큐)가 왜 필요할까요 ?** 

1. GC를 즉시 동기로 돌리면 메인 스레드를 오래 점유할 수 있어요
2. Relay는 _collect()를 제너레이터로 돌리고, root 하나 마킹할 때마다 yield해서 작업을 쪼갭니다.

    → GC를 **중단/재개 가능한 실행 단위**로 만든 형태

3. gcScheduler가 다음 step을 다시 예약하면서 분할 실행해요
4. 이미 GC가 돌고 있으면 중복 예약을 막아요

**> 둘이 함께 있을 때 이점!**

1. 네트워크 이점: 바로 다시 보는 데이터는 재사용 가능해서 재-fetch를 줄입니다.
2. UX 이점: GC를 분할 실행해 프레임 드랍/버벅임을 줄입니다.
3. 메모리 이점: 무한 보관이 아니라 버퍼 크기 + TTL 기준으로 결국 정리됩니다.

**fetch 후 캐싱 관점**

1. fetch 결과가 notify(sourceOperation)로 들어오면 fetchTime이 기록됩니다.
2. 쿼리를 retain하지 않았더라도, releaseBuffer에 공간이 있으면 임시 root로 넣어 “짧은 재사용”이 가능합니다.
3. queryCacheExpirationTime이 지나면 stale로 판단되고, 이후 dispose/GC 경로에서 더 적극적으로 수거됩니다.

**refCount / dispose 중심 흐름**

1. retain(operation) 호출 시 해당 operation의 refCount가 올라갑니다.
2. dispose() 호출 시 refCount가 내려갑니다. 같은 disposable의 dispose()는 한 번만 유효합니다.
3. refCount > 0이면 GC 대상이 아닙니다.
4. refCount == 0이면 일단 releaseBuffer로 들어가서 잠깐 살아있습니다.
5. 버퍼가 꽉 차면 가장 오래된 operation을 버퍼에서 빼고 GC를 스케줄합니다.
6. GC는 남아있는 root(operation)에서 도달 가능한 record만 마킹하고, 나머지를 삭제합니다.
> 같은 쿼리를 2번 retain하면 refCount = 2.  
> 한 번 dispose하면 refCount = 1, 삭제 안 됨.  
> 두 번째 dispose하면 refCount = 0, 바로 삭제가 아니라 버퍼로 이동.  
...
기본적인 Activity 는 아래와 같은 규칙을 따릅니다. 

1. Activity는 UI를 **언마운트하지 않고** visible/hidden 상태로 전환하는 경계에요
2. API는 <Activity mode="visible|hidden">이고, hidden prop은 받지 않아요.
3. 내부적으로는 Activity가 자식을 Offscreen Fiber로 감싸서 동작합니다.
4. mode="hidden"일 때는 숨김 트리를 OffscreenLane으로 지연 렌더(프리렌더)할 수 있습니다.
5. DOM 커밋 시에는 실제로 host 노드에 display: none 등을 적용해 화면에서 숨깁니다.
6. 포털 안의 자식도 Activity 숨김 전파 대상입니다.

가장 중요한점은 useInsertionEffect 에서의 차이점입니다. 

1. Activity의 visible ↔ hidden **가시성 토글 자체만으로는** useInsertionEffect를 다시 실행(언마운트/마운트)하지 않습니다.
2. 하지만 hidden 상태에서라도 자식이 업데이트되어 프리렌더되면, useInsertionEffect는 **cleanup + setup**이 실행될 수 있습니다.
3. 실제 마운트/언마운트(또는 트리 삭제) 시에는 당연히 useInsertionEffect가 동작합니다.

---


문제를 파고들면서 먼저 확인한 건, Relay 훅들이 Activity를 **같은 방식으로** 처리하지 않는다는 점이었어요.


즉 use~~ 훅이 하나의 공통 규칙으로 동작하는 게 아니라, 훅마다 hidden/visible 전환을 다르게 해석하고 있었어요.


핵심 분기점은 useQueryLoader의 Activity 호환 실험 경로였어요.


여기서 핵심은 플래그인 ENABLE_ACTIVITY_COMPATIBILITY 였어요. 


일반적인 relay의 fethcing 훅들은 대부분 useEffect cleanup 에서 dispose하는 방식으로 작동합니다 


ENABLE_ACTIVITY_COMPATIBILITY가 켜지면 useQueryLoader_EXPERIMENTAL을 타는데, 여기서 Relay는 “진짜 unmount”를 잡기 위해 useInsertionEffect cleanup을 사용하려고 했어요.


그런데 여기서 바로 막혔어요. Relay 공식 커밋 메시지에 이미 아래 내용이 명시돼 있었어요.
...
1. [59b2a6e8ae21](https://github.com/facebook/relay/commit/59b2a6e8ae211d87e4dd7f7f2ce784896fd15065) (2024-09-12)
    ...
    커밋 본문에 “suspended tree에서 useInsertionEffect cleanup이 실행되지 않는 React 버그가 있어, 일반 useEffect cleanup에 5분 타이머 fallback을 추가했다”는 설명이 직접 나와요.
...
2. [186c917a0e39](https://github.com/facebook/relay/commit/186c917a0e39c480c663b5707d9ae5b71510cf2d) (2024-09-17)
    ...
    Relay 저장소에 React 동기화로 들어온 커밋인데, visible -> offscreen -> removed 경로에서 insertion cleanup이 누락되던 문제를 수정했다고 적혀 있어요.


    이 커밋은 React 이슈 [#26670](https://github.com/facebook/react/issues/26670), PR [#30954](https://github.com/facebook/react/pull/30954)와 연결돼요.


즉 정리하면, Relay는 처음부터 useInsertionEffect를 “진짜 언마운트 감지”에 쓰고 싶었지만, Suspense/Offscreen 경계에서 cleanup 누락 이슈가 있어서 **타이머 우회 전략**을 같이 넣은 상태였어요.


---


이 지점에서 디버깅 관점이 선명해졌어요.

- Activity는 hidden일 때 트리를 “삭제”하지 않고 “오프스크린”으로 다루려 해요.
- Relay GC는 refCount/retain-dispose 기준으로 “해제 가능한지”를 판단해요.
- 그런데 cleanup 타이밍이 훅마다 다르고(useQueryLoader, useLazyLoadQueryNode, useFragmentInternal), React의 insertion cleanup 누락 이슈까지 겹치면, hidden/visible 전환 중 Store 수명주기와 컴포넌트 기대 수명주기가 어긋나요.

결국 이 크래시는 단순히 “GC가 빨라서”가 아니라,


**Activity(가시성 전환)와 Relay(데이터 해제 타이밍)의 경계가 훅별로 다르게 구현된 상태에서, insertionEffect cleanup 신뢰성 이슈가 겹쳐 터진 문제**였어요.


---


원하면 다음으로 바로 이어서, 이 문단 뒤에 붙일 **“훅별 차이(useQueryLoader/useLazyLoadQueryNode/useFragmentInternal/useMutation) 비교표”**도 동일 톤으로 써줄게.

...
하나씩 보면 정말 작은 요청입니다. 그런데 실제 처리 과정은 전혀 작지 않았습니다.
...
2. 자연어로 수정 요청을 남기기
3. 요청이 큐에 안전하게 저장
4. 워커가 이를 가져가 실제 코드 수정을 시도
5. 변경이 생기면 PR을 만들고 프리뷰 링크를 수집
6. 최종 결과를 다시 운영자에게 알려주는 흐름
...
겉으로 보면 단순한 선택 UX처럼 보이지만, 실제로는**요청이 중간에서 맥락을 잃지 않게 만드는 장치**였습니다.
...
권한 검증이 필요하고, 입력 검증이 필요하고, AI 수정이 들어가고, Git 작업, PR 생성, 프리뷰 확인, 알림까지 붙습니다.


이걸 전부 한 번의 요청에서 처리하려는 순간 실패 지점이 너무 많아지고, 어디서 끊겼는지도 알기 어려워집니다.


또한 AI를 사용하는데 HTTP를 이용한다면 LLM API 호출 비용또한 우리가 부과해야할 문제였습니다. 


그래서 이번에도 큐가 자연스러운 답이 됐습니다.

1. 서버는 요청을 검증해 queued 상태로 저장
2. 워커는 이를 가져가 processing으로 바꾸고
3. 성공하면 completed, 실패하면 failed로 기록
...
요청자나 QA 입장에서는 “처리됐다”는 말보다 “어디서 확인하면 되는가”가 중요했습니다.
